...
> ⚠️ **重要**: 文档必须从 **代码** 派生，不是从规格派生！
>
> **SEED 派生链**: `Spec → Code → Docs`（代码是文档的真相源）

1. 读取派生指导：`$SKILL_DIR/prompts/emit-docs.md`
2. **从代码提取信息生成文档**：
   - 解析代码文件的 JSDoc/注释
   - 提取函数签名、参数类型、返回值
   - 提取 `module.exports` 导出接口
   - 提取配置项和默认值
   - 提取使用示例（如有）
3. 输出到配置路径：`{config.paths.docs}/`

**禁止**: 直接从规格复制内容到文档（这不是派生，是格式转换）
...
| 代码内容 | 文档产物 |
|----------|----------|
| 模块描述 (JSDoc @module) | 简介章节 |
| 函数签名 + JSDoc | API 参考 |
| @example 注释 | 使用示例 |
| @param/@returns | 参数/返回值说明 |
| module.exports | 导出接口列表 |
| 配置对象/默认值 | 配置文档 |
| 错误处理逻辑 | 错误处理说明 |

**文档类型**:
- API 文档：从函数签名和 JSDoc 提取
- 使用指南：从代码示例和 @example 提取
- 配置文档：从配置对象和默认值提取
- 模块文档：从导出和模块结构提取
