# Feature: Task Cancellation (任务取消)

> 状态: implementing
> 版本: 1.0.0
> 技术栈: JavaScript
> 派生路径: skills/mob-seed/lib/ops/
> 优先级: P2
> 预计工作量: 1-2天

## 概述

支持开发过程中的任务取消和状态回滚，确保取消操作不会导致数据不一致或丢失已完成的工作。

## ADDED Requirements

### REQ-001: 取消触发方式

The system SHALL support multiple cancellation triggers.

**触发方式:**

| 方式 | 触发条件 | 处理方式 |
|------|----------|----------|
| 用户主动 | Ctrl+C 或命令取消 | 正常取消流程 |
| 超时 | 超过配置的时间限制 | 超时取消流程 |
| 错误累积 | 连续错误超过阈值 | 自动取消 |
| 外部中断 | 系统信号 (SIGTERM) | 优雅关闭 |

**Scenario: 用户主动取消**
- WHEN 用户发送取消信号
- THEN 标记当前任务为取消中
- AND 执行清理操作
- AND 保存当前进度

**Scenario: 超时取消**
- WHEN 任务执行时间超过限制
- THEN 自动触发取消
- AND 记录超时原因
- AND 保存已完成的部分

**Acceptance Criteria:**
- [ ] AC-001: 支持 Ctrl+C 取消
- [ ] AC-002: 支持超时自动取消
- [ ] AC-003: 支持 API 方式取消

### REQ-002: 安全取消点

The system SHALL define safe cancellation points.

**取消点定义:**

| 阶段 | 取消点 | 安全性 |
|------|--------|--------|
| 分析 | 任何时候 | ✅ 安全 |
| 设计 | 任何时候 | ✅ 安全 |
| 实现 | 文件写入完成后 | ⚠️ 需检查 |
| 测试 | 测试用例完成后 | ✅ 安全 |
| 验证 | 验证命令完成后 | ✅ 安全 |

**Scenario: 在安全点取消**
- WHEN 收到取消信号
- AND 当前不在安全取消点
- THEN 等待到达下一个安全点
- AND 然后执行取消

**Scenario: 强制取消**
- WHEN 等待安全点超过 30 秒
- THEN 强制取消
- AND 标记可能不一致的状态
- AND 记录强制取消日志

**Acceptance Criteria:**
- [ ] AC-004: 识别安全取消点
- [ ] AC-005: 优先在安全点取消
- [ ] AC-006: 支持强制取消

### REQ-003: 状态保存

The system SHALL save state before cancellation.

**保存内容:**

| 内容 | 保存位置 | 用途 |
|------|----------|------|
| 任务进度 | tasks.md | 恢复继续 |
| 工作流状态 | flow-state.json | 流程恢复 |
| 临时文件 | .seed/temp/ | 数据恢复 |
| 日志 | .seed/logs/ | 问题分析 |

**Scenario: 保存取消时状态**
- WHEN 执行取消操作
- THEN 保存当前任务进度到 tasks.md
- AND 保存工作流状态
- AND 生成取消报告

**取消报告格式:**

```markdown
# 任务取消报告

> 取消时间: 2025-01-01 14:30:00
> 取消原因: 用户主动取消

## 取消时状态

| 项目 | 状态 |
|------|------|
| 当前阶段 | 实现 (3/5) |
| 已完成任务 | 5/12 |
| 当前任务 | TASK-006 (进行中) |

## 已完成的工作

- ✅ TASK-001: 需求分析
- ✅ TASK-002: 架构设计
- ✅ TASK-003: complexity-router.js
- ✅ TASK-004: flow-router.js
- ✅ TASK-005: task-sync.js
- ⏸️ TASK-006: 测试代码 (已保存部分进度)

## 恢复指南

要继续此任务，请运行:
```bash
/mob-seed-resume --flow-id=flow-20250101-143000
```
```

**Acceptance Criteria:**
- [ ] AC-007: 保存任务进度
- [ ] AC-008: 保存工作流状态
- [ ] AC-009: 生成取消报告

### REQ-004: 资源清理

The system SHALL clean up resources on cancellation.

**需要清理的资源:**

| 资源类型 | 清理方式 | 保留条件 |
|----------|----------|----------|
| 临时文件 | 删除 | 无 |
| 锁文件 | 释放 | 无 |
| 子进程 | 终止 | 无 |
| 半成品代码 | 回滚或保留 | 可配置 |
| 测试环境 | 重置 | 无 |

**Scenario: 清理资源**
- WHEN 取消操作触发
- THEN 终止运行中的子进程
- AND 释放所有锁
- AND 清理临时文件
- AND 根据配置处理半成品代码

**Acceptance Criteria:**
- [ ] AC-010: 终止子进程
- [ ] AC-011: 释放锁资源
- [ ] AC-012: 清理临时文件

### REQ-005: 取消恢复

The system SHALL support resuming after cancellation.

**Scenario: 从取消点恢复**
- WHEN 用户请求恢复已取消的任务
- THEN 加载保存的状态
- AND 从上次安全点继续
- AND 跳过已完成的步骤

**恢复选项:**

| 选项 | 说明 | 命令 |
|------|------|------|
| 继续 | 从取消点继续 | `--resume` |
| 重新开始 | 从头开始，保留配置 | `--restart` |
| 从阶段开始 | 从指定阶段开始 | `--from-stage=design` |

**恢复交互:**

```
📋 检测到未完成的任务
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

流程 ID: flow-20250101-143000
取消时间: 2025-01-01 14:30:00
取消阶段: 实现 (3/5)
已完成: 5/12 任务

选择操作:
[C] 继续 - 从取消点继续
[R] 重新开始 - 保留配置，重新执行
[S] 从阶段开始 - 选择开始阶段
[D] 放弃 - 清理状态，不恢复

请选择: [C/R/S/D]
```

**Acceptance Criteria:**
- [ ] AC-013: 支持从取消点继续
- [ ] AC-014: 支持重新开始
- [ ] AC-015: 支持选择开始阶段

### REQ-006: 取消日志

The system SHALL log cancellation events.

**日志内容:**

| 字段 | 说明 |
|------|------|
| 时间 | 取消发生时间 |
| 原因 | 取消原因 |
| 触发方式 | 用户/超时/错误 |
| 当前状态 | 取消时的任务状态 |
| 清理结果 | 资源清理情况 |
| 恢复信息 | 如何恢复 |

**Scenario: 记录取消日志**
- WHEN 取消操作完成
- THEN 记录完整的取消日志
- AND 追加到 `.seed/cancellation.log`

**Acceptance Criteria:**
- [ ] AC-016: 记录完整取消信息
- [ ] AC-017: 日志可查询
- [ ] AC-018: 支持日志轮转

## 导出接口

```javascript
module.exports = {
  // 取消触发
  requestCancel,            // (flowId, reason) => void
  registerCancelHandler,    // (handler) => void

  // 安全点
  markSafePoint,            // (flowId) => void
  isAtSafePoint,            // (flowId) => boolean
  waitForSafePoint,         // (flowId, timeout) => Promise<boolean>

  // 状态保存
  saveState,                // (flowId) => StateSnapshot
  generateCancelReport,     // (flowId, snapshot) => string

  // 资源清理
  cleanup,                  // (flowId) => CleanupResult
  terminateSubprocesses,    // (flowId) => void
  releaseLocks,             // (flowId) => void

  // 恢复
  canResume,                // (flowId) => boolean
  resume,                   // (flowId, options) => void
  getResumeOptions,         // (flowId) => ResumeOption[]

  // 日志
  logCancellation,          // (cancelEvent) => void
  getCancellationHistory,   // (flowId?) => CancelEvent[]
};
```

## 配置项

```json
{
  "cancellation": {
    "safePointTimeout": 30,
    "keepPartialWork": true,
    "autoCleanTemp": true,
    "logFile": ".seed/cancellation.log",
    "maxLogSize": "10MB",
    "gracefulShutdownTimeout": 5
  }
}
```

## 依赖

- `core/task-sync.js` - 任务状态
- `workflow/flow-router.js` - 工作流状态

## 测试要点

1. 各种取消触发方式
2. 安全点检测准确性
3. 状态保存完整性
4. 资源清理彻底性
5. 恢复功能正确性
