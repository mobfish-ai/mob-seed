...
- [x] FR-001: 子命令路由 - `/mob-seed <subcommand>` 路由到对应处理器
- [x] FR-002: 智能默认入口 - 无参数时执行全量检查 + 建议行动
- [x] FR-003: 项目检测 - 检查 `.seed/config.json` 并加载配置
- [x] FR-004: 状态收集 - 收集规格/代码/测试/AC 状态
- [x] FR-005: 同步检查 - 规格与代码/测试/文档一致性
- [x] FR-006: 漂移检测 - 识别新增/缺失/不一致问题
- [x] FR-007: 报告输出 - 状态面板 + 建议行动列表

### 非功能需求

- [x] NFR-001: `--quick` 模式 < 3 秒完成
- [x] NFR-002: 默认模式 < 30 秒完成
- [x] NFR-003: 错误信息清晰可操作
