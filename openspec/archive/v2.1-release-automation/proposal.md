...
### 命令统一 (P0)
- [x] AC-001: `/mob-seed` 无参数执行全量检查
- [x] AC-002: 输出包含状态 + 同步 + 守护三部分
- [x] AC-003: `--quick` 模式秒级完成
- [x] AC-004: `--fix` 自动修复简单问题
- [x] AC-005: `--auto` 无需确认自动执行

### 闭环自动化 (P0)
- [x] AC-006: 检测问题后输出建议行动列表
- [x] AC-007: 用户确认后自动执行建议
- [x] AC-008: 操作完成后自动触发下一步检查
- [x] AC-009: Git hooks 集成 pre-commit/pre-push

### 状态进化 (P1)
- [x] AC-010: 测试通过后自动更新 AC 状态
- [x] AC-011: AC 全完成后提示归档
- [x] AC-012: 进度可视化面板

### 版本同步 (P0)
- [x] AC-013: release.sh 更新所有版本文件
- [x] AC-014: CI 检查 tag 版本与文件版本一致
- [x] AC-015: 版本不一致时阻止发布
...
### Mission 集成 (P1)
- [x] AC-019: 各 SEED 阶段执行 Mission 对齐检查
- [x] AC-020: 对齐分数低于阈值时输出警告
- [x] AC-021: --strict 模式下对齐不达标阻止操作
- [x] AC-022: 会话级缓存避免重复加载 Mission
- [x] AC-023: 文件变更时自动失效缓存
