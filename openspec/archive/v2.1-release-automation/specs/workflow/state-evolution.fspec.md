...
状态自动进化器，根据项目实际进展自动更新规格状态，实现 AC 自动标记和生命周期自动转换。
...
- [x] FR-001: 测试结果解析 - 解析测试输出识别通过的 AC
- [x] FR-002: AC 自动更新 - 测试通过后更新规格中的 AC 状态
- [x] FR-003: 生命周期转换 - 满足条件时自动转换规格状态
- [x] FR-004: 归档提示 - AC 全完成后提示可归档
- [x] FR-005: 进度计算 - 计算规格/代码/测试/AC 完成度
- [x] FR-006: 面板渲染 - 渲染可视化进度面板

### 非功能需求

- [x] NFR-001: AC 更新 < 1 秒
- [x] NFR-002: 面板渲染 < 500ms
- [x] NFR-003: 状态更新幂等
...
### 技术约束
- 测试结果需包含 AC 映射注释
- 规格文件使用 Markdown checkbox 格式
- 仅支持 OpenSpec 模式

### 业务约束
- 状态只能单向进化（draft → archived）
- archived 需要 --reopen 才能重新编辑
...
/**
 * 解析测试结果提取 AC 映射
 * @param {string} testOutput - 测试输出
 * @returns {ACResult[]} AC 结果列表
 */
...
### AC-010: 测试通过自动更新 AC
- Given: 测试文件包含 `// AC-001` 注释
- When: 该测试通过
- Then: 规格中 `- [ ] AC-001` 变为 `- [x] AC-001`

### AC-011: AC 全完成提示归档
- Given: 所有 AC 已标记 `[x]`
- When: 执行 `/mob-seed`
- Then: 提示 `所有 AC 已完成，是否归档？ [Y/n]`

### AC-012: 进度可视化面板
- Given: 提案有 4 个规格，8 个 AC
- When: 执行 `/mob-seed`
- Then: 显示进度面板，包含规格/代码/测试/AC 完成度
