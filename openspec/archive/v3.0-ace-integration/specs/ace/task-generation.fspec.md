...
实现从 Proposal 自动派生 tasks.md 的功能。当 Proposal 进入 `implementing` 状态时，自动从 Proposal 内容中提取任务列表并生成 tasks.md 文件。
...
The system SHALL automatically generate tasks.md when proposal enters implementing status.

**Scenario: 提案进入实现阶段**
- GIVEN 一个 review 状态的 Proposal
- WHEN 用户确认开始实现（状态变为 implementing）
- THEN 自动生成 tasks.md
...
- WHEN observation.fspec.md 状态变为 implementing
