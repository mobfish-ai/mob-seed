...
v3.0-ace-integration 提案在进入 implementing 状态时，Phase 2-4 的任务没有关联 fspec，导致：
1. 实施阶段被迫临时创建 fspec
...
| review → implementing | 所有 fspec 必须是 review 或更高状态 |
...
async function transitionProposal(proposalName, newStatus) {
  if (newStatus === 'implementing') {
    ...
    if (!validation.valid) {
      console.error('❌ 提案验证失败，无法进入 implementing 状态');
      console.error('');
