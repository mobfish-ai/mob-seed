# state-evolution 规格

> 版本: 1.0.0
> 创建时间: 2026-01-01
> 最后更新: 2026-01-01
> 优先级: P1

## 概述 (Overview)

状态自动进化器，根据项目实际进展自动更新规格状态，实现 AC 自动标记和生命周期自动转换。

### 目标用户
- SEED 方法论使用者
- 需要自动化状态管理的开发者

### 核心价值
- 测试通过后自动标记 AC
- AC 全完成后自动提示归档
- 进度可视化面板

## 需求 (Requirements)

### 功能需求

- [ ] FR-001: 测试结果解析 - 解析测试输出识别通过的 AC
- [ ] FR-002: AC 自动更新 - 测试通过后更新规格中的 AC 状态
- [ ] FR-003: 生命周期转换 - 满足条件时自动转换规格状态
- [ ] FR-004: 归档提示 - AC 全完成后提示可归档
- [ ] FR-005: 进度计算 - 计算规格/代码/测试/AC 完成度
- [ ] FR-006: 面板渲染 - 渲染可视化进度面板

### 非功能需求

- [ ] NFR-001: AC 更新 < 1 秒
- [ ] NFR-002: 面板渲染 < 500ms
- [ ] NFR-003: 状态更新幂等

## 约束 (Constraints)

### 技术约束
- 测试结果需包含 AC 映射注释
- 规格文件使用 Markdown checkbox 格式
- 仅支持 OpenSpec 模式

### 业务约束
- 状态只能单向进化（draft → archived）
- archived 需要 --reopen 才能重新编辑

## 接口设计 (Interface)

### 状态转换规则

| 当前状态 | 目标状态 | 自动触发条件 |
|---------|---------|-------------|
| draft | review | 规格通过 linter 检查 |
| review | implementing | 用户确认开始实现 |
| implementing | archived | 所有 AC 标记完成 + 测试通过 |

### 进度面板格式

```
╔══════════════════════════════════════════════════════════════╗
║  📊 v2.1-release-automation 进度面板                          ║
╠══════════════════════════════════════════════════════════════╣
║  状态: implementing                                          ║
║  进度: ████████████████░░░░ 80%                             ║
║  ┌─────────────────────────────────────────────────────────┐ ║
║  │ 📋 规格: 4/4 完成                                        │ ║
║  │ 💻 代码: 3/4 已派生                                      │ ║
║  │ 🧪 测试: 12/15 通过                                      │ ║
║  │ ✅ AC:   8/10 完成                                       │ ║
║  └─────────────────────────────────────────────────────────┘ ║
╚══════════════════════════════════════════════════════════════╝
```

### API 接口

```javascript
// lib/workflow/state-evolution.js

/**
 * 解析测试结果提取 AC 映射
 * @param {string} testOutput - 测试输出
 * @returns {ACResult[]} AC 结果列表
 */
function parseTestResults(testOutput);

/**
 * 更新规格中的 AC 状态
 * @param {string} specPath - 规格文件路径
 * @param {ACResult[]} results - AC 结果
 * @returns {Promise<UpdateResult>}
 */
async function updateACStatus(specPath, results);

/**
 * 检查是否可以转换状态
 * @param {string} specPath - 规格文件路径
 * @param {string} targetState - 目标状态
 * @returns {TransitionCheck}
 */
function canTransition(specPath, targetState);

/**
 * 执行状态转换
 * @param {string} specPath - 规格文件路径
 * @param {string} targetState - 目标状态
 * @returns {Promise<void>}
 */
async function transitionState(specPath, targetState);

/**
 * 计算项目进度
 * @param {string} proposalPath - 提案目录路径
 * @returns {Progress}
 */
function calculateProgress(proposalPath);

/**
 * 渲染进度面板
 * @param {Progress} progress - 进度信息
 * @returns {string} 面板字符串
 */
function renderProgressPanel(progress);
```

## 验收标准 (Acceptance Criteria)

### AC-010: 测试通过自动更新 AC
- Given: 测试文件包含 `// AC-001` 注释
- When: 该测试通过
- Then: 规格中 `- [ ] AC-001` 变为 `- [x] AC-001`

### AC-011: AC 全完成提示归档
- Given: 所有 AC 已标记 `[x]`
- When: 执行 `/mob-seed`
- Then: 提示 `所有 AC 已完成，是否归档？ [Y/n]`

### AC-012: 进度可视化面板
- Given: 提案有 4 个规格，8 个 AC
- When: 执行 `/mob-seed`
- Then: 显示进度面板，包含规格/代码/测试/AC 完成度

## 派生产物 (Derived Outputs)

| 类型 | 路径 | 说明 |
|------|------|------|
| 代码 | lib/workflow/state-evolution.js | 状态进化器 |
| 测试 | test/workflow/state-evolution.test.js | 单元测试 |
| 文档 | docs/api/state-evolution.md | API 文档 |
